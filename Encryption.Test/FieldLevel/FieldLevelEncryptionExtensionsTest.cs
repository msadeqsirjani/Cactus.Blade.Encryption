...
using Cactus.Blade.Encryption.FieldLevel;
...
namespace Encryption.Test.FieldLevel
{
    [TestClass]
    public class FieldLevelEncryptionExtensionsTest
    {
        [TestMethod]
        public void EncryptXmlWorks()
        {
            ...
            mockCrypto.Setup(m => m.GetEncryptor(It.IsAny<object>().ToString())).Returns(mockEncryptor.Object);
            mockEncryptor.Setup(m => m.Encrypt(It.IsAny<string>())).Returns((string plainText) => Base64.Encrypt(plainText));

            const string xml = "<foo bar=\"123\"><baz>456</baz><baz>789</baz><qux><garply grault=\"abc\" /></qux></foo>";
            var keyIdentifier = string.Empty;

            var encryptedXml = mockCrypto.Object.EncryptXml(xml, new[] { "/foo/@bar", "/foo/baz", "/foo/qux" }, keyIdentifier);

            Assert.AreNotEqual(encryptedXml, "<foo bar=\"MTIz\"><baz>NDU2</baz><baz>Nzg5</baz><qux>PGdhcnBseSBncmF1bHQ9ImFiYyIgLz4=</qux></foo>");
        ...
        public async Task EncryptXmlAsyncWorks()
        {
            ...
            mockAsyncCrypto.Setup(m => m.GetAsyncEncryptor(It.IsAny<object>().ToString())).Returns(mockAsyncEncryptor.Object);
            ...
            Assert.AreEqual(encryptedXml, "<foo bar=\"MTIz\"><baz>NDU2</baz><baz>Nzg5</baz><qux>PGdhcnBseSBncmF1bHQ9ImFiYyIgLz4=</qux></foo>");
        ...
        public void DecryptXmlWorks()
        {
            ...
            mockCrypto.Setup(m => m.GetDecryptor(It.IsAny<object>().ToString())).Returns(mockDecryptor.Object);
            mockDecryptor.Setup(m => m.Decrypt(It.IsAny<string>())).Returns((string plainText) => Base64.Decrypt(plainText));

            const string xml = "<foo bar=\"MTIz\"><baz>NDU2</baz><baz>Nzg5</baz><qux>PGdhcnBseSBncmF1bHQ9ImFiYyIgLz4=</qux></foo>";
            var keyIdentifier = string.Empty;

            var decryptedXml = mockCrypto.Object.DecryptXml(xml, new[] { "/foo/@bar", "/foo/baz", "/foo/qux" }, keyIdentifier);

            Assert.AreEqual(decryptedXml, "<foo bar=\"123\"><baz>456</baz><baz>789</baz><qux><garply grault=\"abc\" /></qux></foo>");
        ...
        {
            public static string Encrypt(string value) => Encoding.UTF8.GetBytes(value).ToBase64String();
